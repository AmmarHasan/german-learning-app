...
export function ChapterList({
  ...
  const currentChapters = level === "A1" ? chapters : a2Chapters;
    ...
    <div className="w-full max-w-2xl mx-auto py-12 px-4 sm:px-6">
      <div className="mb-10 text-center">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-lg max-w-md mx-auto mb-8">
          Master the vocabulary chapter by chapter using spaced repetition
          flashcards.
        </p>

        <div className="flex justify-center mb-8">
          <div className="bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-2xl flex">
            <button
              onClick={() => setLevel("A1")}
              className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all ${level === "A1" ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300"}`}
            >
              A1 Level
            </button>
            <button
              onClick={() => setLevel("A2")}
              className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all ${level === "A2" ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300"}`}
            >
              A2 Level
            </button>
          </div>
        ...
        <AnimatePresence mode="wait">
          {currentChapters.map((chapter, index) => {
            ...
            const progressPercent =
              total > 0 ? (masteredCount / total) * 100 : 0;

            return (
              <motion.button
                key={`${level}-${chapter.id}`}

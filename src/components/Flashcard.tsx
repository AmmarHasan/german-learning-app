...
export function Flashcard({ word, active, onSwipe, index }: FlashcardProps) {
  ...
  function getArticleColor() {
    switch (word.article) {
      ...
      case "die":
        return "text-pink-600 bg-pink-100 border-pink-200 dark:text-pink-400 dark:bg-pink-900/30 dark:border-pink-800";
      case "das":
          ...
          {studyDirection === "de-en" ? (
            <>
              {word.pos === "noun" && word.article && (
                <span
                  className={cn(
                    "px-4 py-1.5 rounded-full text-sm font-bold uppercase tracking-wider mb-4 border",
                    getArticleColor(),
                  )}
                >
                  {word.article}
            ...
            <>
              <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider mb-4">
                {word.pos}
              </span>
              <h2 className="text-4xl sm:text-5xl font-bold tracking-tight text-slate-900 dark:text-white">
                {word.english}
